...
class LabApp(ctk.CTk):
    ...
    def _show_catalog_view(self):
        ...
        # Load and display table data after a short delay to ensure UI is ready
        # This prevents any timing issues with widget creation
        self.after(50, self._load_catalog_table)
    ...
    def _filter_catalog_table(self):
        ...
        for item in filtered_items:
            try:
                ...
                print(f"Error creating catalog row: {e}")
                import traceback
                traceback.print_exc()
                continue
    ...
    def _create_catalog_table_row(self, item: Dict, student_lookup: Dict):
        ...
        is_overdue = item["id"] in self.overdue_item_ids
        
        # Row frame: Each row is a separate frame with bottom border
        # Overdue items get red border for visual alert
        row_frame = ctk.CTkFrame(
            self.catalog_table_body,
            fg_color="#1a0a0a" if is_overdue else "transparent",  # Subtle red background for overdue
            height=56,
            border_width=2 if is_overdue else 0,
            border_color="#ef4444" if is_overdue else "transparent"
        )
