...
class LabApp(ctk.CTk):
    ...
    def _load_staff(self):
        """Load staff/issuers into dropdown."""
    ...
    def _finalize_issue(self):
        ...
        # Get issuer (staff member)
        issuer_value = self.issuer_dropdown.get()
        if not issuer_value:
            self._show_error("Please select an issuer (staff member).", "No Issuer Selected")
            return
        ...
        if not selected_issuer:
            self._show_error("Invalid issuer selection.", "Error")
            return
        ...
        # Create transaction with issuer_id
        item_ids = [item["id"] for item in self.cart_items]
        transaction_id = self.db.create_transaction(selected_student["id"], item_ids, issuer_id=selected_issuer["id"])
    ...
    def _show_catalog_view(self):
        ...
        unique_courses = sorted(set([inv.get("course", "N/A") for inv in all_inventory if inv.get("course")]))
        ...
        # Ensure search entry is empty and filter is set to "All" before loading
        ...
        # Ensure filter is initialized to "All"
        self.catalog_filter_status = "All"
        ...
        for item in all_items:
            ...
            if self.catalog_filter_course != "All":
                ...
                if isinstance(inventory, dict):
                    item_course = inventory.get("course", "N/A")
                else:
                    ...
                    if inventory_id:
                        ...
                        item_course = inv.get("course", "N/A") if inv else "N/A"
                    else:
                        item_course = "N/A"
                
                if item_course != self.catalog_filter_course:
                    continue
    ...
    def _create_catalog_table_row(self, item: Dict, student_lookup: Dict):
        ...
        # Note: CustomTkinter doesn't allow "transparent" for border_color, so we use None
        if is_overdue:
            ...
            row_frame = ctk.CTkFrame(
                ...
                fg_color="transparent",
                border_width=0,
                border_color=None,  # Use None instead of "transparent"
                height=56
            )
        ...
        if "inventory" in item and item["inventory"]:
            inventory_name = item["inventory"].get("name", "Unknown")
            inventory_course = item["inventory"].get("course", "N/A")
        elif "inventory_id" in item:
            inv_list = self.db.get_all_inventory()
            for inv in inv_list:
                if inv["id"] == item["inventory_id"]:
                    inventory_name = inv["name"]
                    inventory_course = inv.get("course", "N/A")
                    break
