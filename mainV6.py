...
class LabApp(ctk.CTk):
    ...
    def _filter_catalog_table(self):
        ...
        print(f"Catalog: Retrieved {len(all_items)} items, filter='{self.catalog_filter_status}', search='{search_query}'")
        for idx, item in enumerate(all_items):
            print(f"  Item {idx+1}: ID={item.get('id')}, Serial={item.get('serial_number')}, Status={item.get('status')}, Inventory={item.get('inventory', {}).get('name', 'N/A')}")
    ...
    def _load_catalog_table(self):
        ...
        """
        # Ensure filter is set to "All" for initial load
        if not hasattr(self, 'catalog_filter_status'):
            self.catalog_filter_status = "All"
    ...
    def _create_catalog_table_row(self, item: Dict, student_lookup: Dict):
        ...
        row_frame.grid_columnconfigure(0, weight=0, minsize=300)
